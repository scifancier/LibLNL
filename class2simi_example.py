...
# @Time         : 8/7/23 6:47 pm
# @File         : example.py
# @Description  : Example to run Class2Simi with LibLNL
...
from LibLNL.algorithm.class2simi import class2simi_learning
...
def main():
    ...
    est_optimizer = torch.optim.__dict__[args.optimizer](model.parameters(), **est_optim_args)

    # Estimate transition matrix
    transition_matrix = estimate_t_anchor(args.est_epochs, train_loader, est_loader, val_loader, args.batch_size,
                                          train_data_num, args.num_class, model, est_optimizer)
    # # torch.save(model.state_dict(), 'est_epoch.pth')
    #
    # model.load_state_dict(torch.load('est_epoch.pth'))
    # Forward training
    ...
    class2simi_learning(args.epochs, model, train_loader, val_loader, test_loader, transition_matrix, optimizer, scheduler)
