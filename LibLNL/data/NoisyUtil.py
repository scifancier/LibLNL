...
def dataset_split(train_images, train_labels, noise_rate=0.5, noise_type='symmetric', split_per=0.9, random_seed=1, num_classes=10):
    ...
    if noise_type == 'pairflip':
        ...
        noisy_labels, real_noise_rate, transition_matrix = noisify_pairflip(train_labels, noise_rate, random_state=random_seed, nb_classes=num_classes)
    elif noise_type == 'symmetric':
        train_labels = train_labels[:, np.newaxis]
        noisy_labels, real_noise_rate, transition_matrix = noisify_multiclass_symmetric(train_labels, noise_rate, random_state=random_seed, nb_classes=num_classes)
    elif noise_type == 'multiflip':
        train_labels = train_labels[:, np.newaxis]
        noisy_labels, real_noise_rate, transition_matrix = CCN_generator_multiflip(train_labels, noise_rate, random_state=random_seed, nb_classes=num_classes)
        train_labels = train_labels[:, np.newaxis]
    elif noise_type == 'random':
        noisy_labels, real_noise_rate, transition_matrix = CCN_generator_random(train_labels, noise_rate, random_state=random_seed, nb_classes=num_classes)
    elif noise_type == 'instance':
        train_labels = torch.from_numpy(train_labels)
        noisy_labels, real_noise_rate, transition_matrix = get_instance_noisy_label(n=noise_rate, dataset=dataset, labels=train_labels, nb_classes=num_classes, feature_size=feature_size, norm_std=norm_std, random_state=random_seed)
    else:
        train_labels = train_labels[:, np.newaxis]
        noisy_labels = train_labels[:, np.newaxis]
        transition_matrix = np.eye(num_classes)
        real_noise_rate = 0.

    print('Noise type: {}.'.format(noise_type))
    print('Noise rate: {}.'.format(real_noise_rate))
    ...
    return train_set, val_set, train_labels, val_labels, train_clean_labels, val_clean_labels, transition_matrix
...
class Train_Dataset_Index(Dataset):
    def __init__(self, data, labels, transform=None, target_transform=None):
        self.train_data = np.array(data)
        self.train_labels = np.array(labels)
        self.length = len(self.train_labels)
