...
class NormalizedGeneralizedCrossEntropy(torch.nn.Module):
    def __init__(self, num_classes=10, scale=1.0, q=0.7):
...
class GCEandMAE(torch.nn.Module):
    def __init__(self, beta=1.0, num_classes=10, q=0.7):
...
class NGCEandNCE(torch.nn.Module):
    def __init__(self, alpha=1.0, beta=1.0, num_classes=10, q=0.7):
...
def robust_loss_learning(robust_loss, epochs, model, train_loader, val_loader, test_loader, optimizer, scheduler):
    val_acc = []
    test_acc = []
